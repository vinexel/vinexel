...
use Iconic\Core\Modules\Container\Container;
...
class Router
{
    ...
    protected static $middleware = []; // Middleware global
    protected static $routeGroups = []; // Menyimpan grup rute
    ...
    public static function dispatch()
    {
        ...
        foreach (self::$routes as $route) {
            if ($route['method'] === $requestMethod && preg_match(self::convertToRegex($route['uri']), $requestUri, $matches)) {
                array_shift($matches); // Menghapus URL yang dicocokkan
        ...
        // Jika tidak ada rute yang cocok, panggil handler 404
